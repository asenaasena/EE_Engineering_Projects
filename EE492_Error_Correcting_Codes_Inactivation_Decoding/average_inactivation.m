...
size_stopping_col=zeros(length(p),1);
size_stopping_row=zeros(length(p),1);
 ...
 while( any(new_matrice(:))) %if it is full-rank but needed inactivation
    ...
    if(any(new_matrice(:))~=0)%which means peeling was not enough
     ...
     if(flag==0) %where it starts inactivation
         size_stopping_row(err)=size_stopping_row(err)+size(new_matrice,1);
         size_stopping_col(err)=size_stopping_col(err)+size(new_matrice,2);
...
if(flag==1)
    ...
    num_in=[num_in length(mem_freeze)]
%
%%%%%%%%%%%%%%%%%
